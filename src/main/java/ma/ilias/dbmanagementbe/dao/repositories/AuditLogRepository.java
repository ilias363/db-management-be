...
public interface AuditLogRepository extends JpaRepository<AuditLog, Long> {
    ...
    @Query("SELECT COUNT(a) FROM AuditLog a WHERE a.auditTimestamp >= DATE_SUB(NOW(), INTERVAL 1 DAY)")
    long countAuditsLast24Hours();

    @Query(value = "SELECT action_type FROM audit_logs GROUP BY action_type ORDER BY COUNT(action_type) DESC LIMIT 1", nativeQuery = true)
    String findMostCommonActionString();

    @Query(value = "SELECT COUNT(*) / (DATEDIFF(CURDATE(), MIN(DATE(audit_timestamp))) + 1.0) FROM audit_logs WHERE audit_timestamp IS NOT NULL", nativeQuery = true)
